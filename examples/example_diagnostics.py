...
Potential Scale Reduction Factor (R̂, PSRF)
    ...
    Rule of thumb: R̂ < 1.1 indicates good convergence.
...
for i in range(S.shape[0]):
    ess_ok  = "✓" if ess_vals[i] >= 200 else "✗"
    psrf_ok = "✓" if u_psrf_vals[i] < 1.1 else "✗"
    print(
        f"  {i:>5}  {ess_vals[i]:>8.1f}  {u_psrf_vals[i]:>8.4f}  "
        f"ESS{ess_ok} PSRF{psrf_ok}"
    )
print(f"\n  Multivariate PSRF: {m_psrf_val:.4f}  "
      f"({'converged ✓' if m_psrf_val < 1.1 else 'not converged ✗'})")
