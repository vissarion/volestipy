...
# ─── helper: reconstruct the full n×n doubly stochastic matrix ──────────────

def reduced_to_full(x: np.ndarray, n: int) -> np.ndarray:
    """
    Reconstruct the full n×n doubly stochastic matrix from a reduced
    ...
    """
    M = np.zeros((n, n))
    # Fill the top-left (n-1)×(n-1) block
...
print(f"  Sample matrix shape: {samples.shape}  (d × n_samples)")
...
print("  Reconstructed 3×3 doubly stochastic matrix:")
